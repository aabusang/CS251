...
//create a new accountId and use that to create a new account
    ...
    public void submitToRobbery(double amount) {
        ...
        //Determine the amount to withdraw for the bad guy and try withdrawing it
        //throw insuf. funds when a given member can't pay then reduces the number of account by one
        //and loop again for the next account
        int nOfAccounts = accounts.size();
        for (Account account : accounts) {
            double forRobber = amount / nOfAccounts;
            try {
                account.withdraw(forRobber);
