...
public class RequestManager implements Manager {
     ...
     */
    // TODO: Fill this in with the above logic
    public void processDepositRequest(Person person, double amount) { //I THREW THIS(notoldEnough) EXCEPTIONS, NOT SURE IF IT'S RIGHT

        Account account_a;
        if (!securityManager.isBanned(person)){
                try {
                    account_a = accountsManager.lookupAccount(membershipManager.lookupMember(person).getAccountId());
     ...
     */
    // TODO: Fill this in with the above logic
    public void processWithdrawRequest(Person person, double amount) {
        if (!securityManager.isBanned(person)){
            Account account_a;
            try {
                account_a = accountsManager.lookupAccount(membershipManager.lookupMember(person).getAccountId());
...
//                accountNotFound.printStackTrace();
     ...
     * and account.
     * Then they can proceed to make a judgement on whether or not
     * they qualify for a loan.
     *
     * If they do qualify then the loan should be
    ...
    public void processNewLoanRequest(Person person, double amount) {
        ...
        if (!securityManager.isBanned(person)){
            try{
                ...
                Account account_a = accountsManager.lookupAccount(member.getAccountId());

                if (loansManager.processNewLoanRequest(person, member, account_a, amount)){
                    account_a.addLoan(amount);
                }else {
                    System.out.println("Not Qualified for a loan");
                }
            }catch (MemberNotFound memberNotFound) {
//                memberNotFound.printStackTrace();
                try {
                    membershipManager.addMember(accountsManager, person, amount);
                    processNewLoanRequest(person, amount); //recursively call processNewLoanRequest
                } catch (NotOldEnough notOldEnough) {
                    notOldEnough.printStackTrace();
                }
            } catch (AccountNotFound accountNotFound) {
    ...
    public void processPayLoanRequest(Person person, double amount) {
        if (!securityManager.isBanned(person)){
            try {
                Member member  = membershipManager.lookupMember(person);
                Account account_a = accountsManager.lookupAccount(member.getAccountId());
                account_a.payLoan(amount);
            }
            catch (MemberNotFound memberNotFound) {
//                memberNotFound.printStackTrace();
                try{
                    membershipManager.addMember(accountsManager, person, person.getMonthlyIncome());
                    processPayLoanRequest(person, person.getMonthlyIncome());
                } catch (NotOldEnough notOldEnough) {
                    notOldEnough.printStackTrace();
                }
            }
    ...
    public void processEndOfDay(double membershipFee,
        ...
        catch (AccountNotFound | CannotPayFees exc) {
            System.out.println(exc);
        }
